...
def main(annot_path):
    ...
    print("Found {} bboxes of which {} has keypoints annotated. ({:.2f} %)".format(n_bboxes, n_kpts, n_kpts / n_bboxes * 100))
    ...
    print("Found {} ({:.2f} %) unique images with bboxes, last annotated image is {}".format(
        len(unique_imgs_w_bboxes),
        len(unique_imgs_w_bboxes) / len(data["images"]) * 100,
        unique_imgs_w_bboxes[-1],
    ))
    print("Found {} ({:.2f} %) unique images with keypoints, last annotated image is {}".format(
        len(unique_imgs_w_kpts),
        len(unique_imgs_w_kpts) / len(data["images"]) * 100,
        unique_imgs_w_kpts[-1],
    ))
    print("{:.2f} % of bboxes have keypoints".format(
